...
ID3D11Buffer* Renderer::m_pLightBuffer{}; // ライト情報
ID3D11Buffer* Renderer::m_pMaterialBuffer{}; // マテリアル情報
...
HRESULT Renderer::Init()
{
	...
	m_pDeviceContext->VSSetConstantBuffers(0, 1, &m_pWorldBuffer); //00
	if (FAILED(hr)) return hr;

	hr = m_pDevice->CreateBuffer(&bufferDesc, NULL, &m_pViewBuffer);
	m_pDeviceContext->VSSetConstantBuffers(1, 1, &m_pViewBuffer);  //01
	if (FAILED(hr)) return hr;

	hr = m_pDevice->CreateBuffer(&bufferDesc, NULL, &m_pProjectionBuffer);
	m_pDeviceContext->VSSetConstantBuffers(2, 1, &m_pProjectionBuffer); //02
	if (FAILED(hr)) return hr;
	...
	m_pDeviceContext->VSSetConstantBuffers(3, 1, &m_pLightBuffer); //03
	if (FAILED(hr)) return hr;
	...
	m_pDeviceContext->VSSetConstantBuffers(4, 1, &m_pMaterialBuffer); //04
	m_pDeviceContext->PSSetConstantBuffers(4, 1, &m_pMaterialBuffer); //04
	if (FAILED(hr)) return hr;
...
{
	m_pDeviceContext->UpdateSubresource(m_pMaterialBuffer, 0, NULL, &pMaterial, 0, 0);
}
//--------------------------------------------------------------------------------------
// UV情報を設定
//--------------------------------------------------------------------------------------
