...
bool Application::InitApp()
{
	...
	// ウィンドウサイズを調整
	auto style = WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_THICKFRAME;
...
LRESULT CALLBACK Application::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	static bool hasWindowedRect = false;
	//新 
	...
	case WM_KEYDOWN: //キー入力があったメッセージ
		...
		else if (LOWORD(wParam) == VK_F11)
		{
			...
			if (isFullscreen) {
				// フルスクリーンモードに切り替える前のウィンドウの位置とサイズを保存
				//新 
				if (!hasWindowedRect) {
	...
	case WM_SIZING:
	{
		...
		const float targetAspect = 1920.f / 1080.f;
		...
		case WMSZ_RIGHT:
		{
			const int newHeight = static_cast<int>(width / targetAspect);
		...
		case WMSZ_BOTTOM:
		{
			const int newWidth = static_cast<int>(height * targetAspect);
